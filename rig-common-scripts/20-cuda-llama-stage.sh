...
if [ -d /opt/cache/llama-prebuilt ] && [ -f /opt/cache/llama-prebuilt/bin/llama-server ]; then
    ...
    echo "=== COMPILO __LLAMA_FLAVOR__ sul target (nessuna build pre-compilata in cache) ==="
    [ "__LLAMA_FLAVOR__" = "beellama" ] && echo "!!! FA_ALL_QUANTS attivo: compilazione LUNGA (30-60+ min)." >&2
    cd /opt
    git clone --depth 1 __LLAMA_REPO__ llama.cpp
    cd llama.cpp
    cmake -B build \
        -DGGML_CUDA=ON \
